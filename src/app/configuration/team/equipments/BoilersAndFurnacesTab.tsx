...
const BoilersAndFurnacesTab = ({ branchesData }: IProps) => {
	...
	const {
		...
		!userInfo?.company?.id
			? null
			: [IApiEndpoint.GET_STATIONARY_EQUIPMENTS_BY_CATEGORY_PAGINATED, { page, limit, companyId: userInfo?.company?.id, search, branch: "", category: StationaryCombustionAddVariant.BOILERS_FURNACES }],
		swrFetcher,
		...
		<div className="w-full">
			<div className="grid grid-cols-1 md:grid-cols-2">
				...
				<div className="flex items-center justify-end">
					<StationaryCombustionApplianceModal variant={StationaryCombustionAddVariant.BOILERS_FURNACES} mutate={refetch} />
				</div>
			</div>
			<div className="mt-5">
