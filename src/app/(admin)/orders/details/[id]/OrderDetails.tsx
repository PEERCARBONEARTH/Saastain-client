...
import { BreadcrumbItem, Breadcrumbs, Button, Card, CardBody, CardFooter, CardHeader, Chip, Divider, Image, Progress, Dropdown, DropdownItem, DropdownMenu, DropdownTrigger, cn, Skeleton } from "@heroui/react";
...
const OrderDetails: FC<IProps> = ({ id }) => {
	...
	const computedActualCost = useMemo(() => {
		if (orderQuoteDetails) {
			return orderQuoteDetails?.variantsInfo?.reduce((total, item) => {
	...
	const variants = useMemo(() => {
		if (orderQuoteDetails) {
			return orderQuoteDetails?.variantsInfo?.map((item) => {
				return {
					...item,
					id: item.code,
				};
					...
					<div className="px-3 py-3 border rounded-xl mb-2">
						...
						<div className="">
							<div className="w-full flex items-center justify-between gap-5">
								...
								<div className="w-full space-y-3">
									<Progress
										value={orderQuoteDetails ? 100 : 50}
										classNames={{
											indicator: orderQuoteDetails ? "bg-green-600" : "bg-yellow-300",
											track: "bg-gray-200",
										}}
										aria-label="rfq"
									/>
								...
								<div className="flex items-center gap-2">
									{orderQuoteDetails ? null : (
										<>
											...
											{currentOrderSiteVisitSchedule && !currentOrderSiteVisitSchedule?.isApproved && (
												<ConfirmSiteVisitModal siteVisitItem={currentOrderSiteVisitSchedule} mutate={refetchSchedule} orderId={id} />
											)}
										</>
									)}
					...
					<div className="mt-8 grid grid-cols-1 md:grid-cols-12 gap-5">
						<div className="col-auto md:col-span-8">
							<AppTable<IVariantsInfoItem>
								...
								data={orderQuoteDetails ? variants : []}
										...
										{orderQuoteDetails && (
											<>
												<div className="w-full flex items-center justify-between">
													...
													<h3 className="text-gray-800 text-sm font-semibold">{orderQuoteDetails?.variantsInfo?.length ?? 0}</h3>
												</div>
												<div className="w-full flex items-center justify-between">
												...
												<div className="w-full flex items-center justify-between">
													...
													<h3 className="text-gray-800 text-sm font-semibold">{formatCurrency(orderQuoteDetails?.maintenanceCost)}</h3>
												</div>
												<div className="w-full flex items-center justify-between">
													<h3 className="text-gray-500 text-sm font-semibold">Installation Cost</h3>
													<h3 className="text-gray-800 text-sm font-semibold">{formatCurrency(orderQuoteDetails?.installationCost)}</h3>
												</div>
												<div className="w-full flex items-center justify-between">
													<h3 className="text-gray-500 text-sm font-semibold">Total Cost</h3>
													<h3 className="text-gray-800 text-sm font-semibold">{formatCurrency(orderQuoteDetails?.totalCost)}</h3>
												</div>
												<div className="w-full flex items-center justify-between">
													<h3 className="text-gray-500 text-sm font-semibold">Total Area</h3>
													<h3 className="text-gray-800 text-sm font-semibold">{orderQuoteDetails?.totalArea}</h3>
												</div>
											</>
										)}
										{!orderQuoteDetails && (
								...
								<Card shadow="none" className="bg-transparent border">
									...
									<CardBody>
										<div className="w-full space-y-2">
											{orderQuoteDetails && orderQuoteDetails?.documents && orderQuoteDetails?.documents?.length > 0 ? (
												<>
													{orderQuoteDetails?.documents?.map((doc) => (
														<DocumentItem {...doc} />
													))}
												</>
											) : (
									...
									<CardFooter>
										<div className="w-full flex items-center justify-end">
											<Button isDisabled={!orderQuoteDetails} color="primary" variant="bordered">
												Download Invoice
											</Button>
										</div>
									</CardFooter>
