...
const bulkColumns: ColumnDef<IBulkAddHeatAndCoolingData, any>[] = [
	...
	bulkHeatColumnHelper.accessor("emissionSource", {
...
const BulkAddHeatAndCoolingData = () => {
	...
	const onClickSave = () => {
		...
		const validData = data
			...
			.map((row) => {
				const { date, emissionSource, units, amount } = row;
	...
	const onConfirm = async () => {
		...
		try {
			...
			const resp = await saveBulkHeatAndCoolingData(account?.company?.id, dataToBeSaved);
				...
				<Tabs selectedKey={selectedTab} disabledKeys={["preview"]} color="primary" onSelectionChange={(key) => onTabChange(new Set([key]))}>
					<Tab key={"add-data"} title={"Add Data"}>
						<div className="flex items-center justify-between">
							<h1 className="text-3xl font-semibold">Heat & Cooling</h1>
...
export default BulkAddHeatAndCoolingData;
