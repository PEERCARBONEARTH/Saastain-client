...
interface BoilersAndFurnacesAddData {
	date: string;
...
interface BoilersAndFurnacesWithEmissions extends BoilersAndFurnacesAddData {
	id: string;
	c02KgEmitted: number;
...
const BoilersAndFurnaces = () => {
	const [editedRows, setEditedRows] = useState<Record<string, BoilersAndFurnacesAddData>>({}); // { [rowId]: boolean }
	const [validRows, setValidRows] = useState<Record<string, BoilersAndFurnacesAddData>>({}); // { [rowId]: [x: string]: boolean }
	const [data, setData] = useState<BoilersAndFurnacesAddData[]>([]);
	...
	const [dateToBeSaved, setDateToBeSaved] = useState<BoilersAndFurnacesWithEmissions[]>([]);
	...
	const bulkColumnHelper = createColumnHelper<BoilersAndFurnacesAddData>();

	const bulkColumns: ColumnDef<BoilersAndFurnacesAddData, any>[] = [
		bulkColumnHelper.accessor("date", {
			header: "Accounting Date",
			cell: AppEditableCell<BoilersAndFurnacesAddData>,
			meta: {
				data: {
					type: "datepicker",
		...
		bulkColumnHelper.display({
			id: "actions",
			header: "Actions",
			cell: AppEditableTableActionBtns<BoilersAndFurnacesAddData>,
		}),
	];
	...
	const renderPreviewCell = useCallback((item: BoilersAndFurnacesWithEmissions, columnKey: Key) => {
		const value = item[columnKey as keyof BoilersAndFurnacesWithEmissions] as string | number;
	...
	return (
		<>
			<Breadcrumbs>
				...
				<BreadcrumbItem>Boilers And Furnaces</BreadcrumbItem>
			</Breadcrumbs>
				...
				<Tabs selectedKey={selectedTab} disabledKeys={["preview"]} color="primary" onSelectionChange={(key) => onTabChange(new Set([key]))}>
					<Tab key={"add-data"} title={"Add Data"}>
						<div className="flex items-center justify-between">
							<h1 className="text-3xl font-semibold">Boilers And Furnaces</h1>
						...
						<AppEditableTable<BoilersAndFurnacesAddData>
							columns={bulkColumns}
							data={data}
							defaultData={[]}
							setData={setData}
					...
					<Tab key={"preview"} title={"Preview"}>
						...
						<AppTable<BoilersAndFurnacesWithEmissions>
...
export default BoilersAndFurnaces;
