...
const EditProcessingEmissionsData: FC<IProps> = ({ id, scopeId }) => {
	...
	useEffect(() => {
		if (initialData) {
			...
			setValue("equipmentName", { label: initialData?.emissionName, value: initialData?.emissionName });
			setValue("wasteGas", initialData?.wasteGas);
	...
	const onConfirm = async () => {
		...
		const toastId = toast.loading("Updating Process Emissions data...");

		try {
			const resp = await updateProcessingEmissionsData(dataToSave);

			if (resp?.status === "success") {
				toast.success("Process Emission updated successfully", { id: toastId });
				reset();
				setOpenConfirmModal(false);
				router.push(AppEnumRoutes.APP_DATA_LIST);
			} else {
				console.log(resp);
				toast.error("Failed to update Process Emission", { id: toastId });
			}
		} catch (err) {
			console.error(err);
			toast.error("Failed to update Process Emission", { id: toastId });
...
export default EditProcessingEmissionsData;
