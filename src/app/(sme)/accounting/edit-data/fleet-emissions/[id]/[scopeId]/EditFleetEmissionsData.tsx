...
const EditFleetEmissionsData: FC<IProps> = ({ id, scopeId }) => {
	// define the form
	...
	const { queryFleetInfo, saveFleetInfo, updateVehicleEmissionsData } = useAccountingDataUtils();
	...
	const { data: initialData } = useSWR<IScopeOneFleet & { date: string }>([IApiEndpoint.GET_SCOPE_ONE_VEHICLE_EMISSIONS_DATA, { scopeId }], swrFetcher, { keepPreviousData: true });

	useEffect(() => {
		if (initialData) {
			setValue("date", new Date(initialData?.date));
			setValue("amountOfDistance", parseFloat(String(initialData?.distanceCovered)));
		}
	}, [initialData]);

	useEffect(() => {
		if (initialData && loadedFleetTypes) {
			setValue("fleetType", initialData?.typeLevel1);
		}
	}, [initialData, loadedFleetTypes]);

	useEffect(() => {
		if (initialData && loadedFleetCategories) {
			setValue("fleetCategory", initialData?.typeLevel2);
		}
	}, [initialData, loadedFleetCategories]);

	useEffect(() => {
		if (initialData && loadedTypeOfFuels) {
			setValue("typeOfFuel", initialData?.fuelType);
		}
	}, [initialData, loadedTypeOfFuels]);

	useEffect(() => {
		if (initialData && loadedUnitOfDistance) {
			setValue("unitOfDistance", loadedUnitOfDistance?.[0]?.value);
		}
	}, [initialData, loadedUnitOfDistance]);
	...
	const onConfirm = async () => {
		...
		try {
			const resp = await updateVehicleEmissionsData(dataToSave);
	...
	return (
		<AuthRedirectComponent>
			<Card className="mt-150 p-6 bg-[#E4FCE6] h-full overflow-y-scroll">
				...
				<FleetEmissionConfirmModal isOpen={openConfirmModal} setIsOpen={setOpenConfirmModal} values={modalValues} onConfirm={onConfirm} isSaving={isSaving} actionType="update" />
			</Card>
		</AuthRedirectComponent>
	);
};

export default EditFleetEmissionsData;
