...
interface IBulkFleetEmissionsData {
	date: string;
	fleetType: string;
...
const BulkAddFleetEmissionsData = () => {
	const [editedRows, setEditedRows] = useState<Record<string, IBulkFleetEmissionsData>>({}); // { [rowId]: boolean }
	const [validRows, setValidRows] = useState<Record<string, IBulkFleetEmissionsData>>({}); // { [rowId]: [x: string]: boolean }
	const [data, setData] = useState<IBulkFleetEmissionsData[]>([]);
	...
	const [dateToBeSaved, setDateToBeSaved] = useState<IBulkFleetEmissionsDataWithEmissions[]>([]);
	...
	async function loadFleetTypes<T = any>(table: Table<T>, rowId: string) {
		...
		try {
			...
			if (resp?.status === "success") {
				...
				if (Array.isArray(info)) {
					...
					tableMeta?.updateCustomOptions(rowId, "fleetType", options);
				}
			}
		} catch (err) {
			console.error(err);
	...
	const bulkColumnHelper = createColumnHelper<IBulkFleetEmissionsData>();

	const bulkColmns: ColumnDef<IBulkFleetEmissionsData, any>[] = [
		bulkColumnHelper.accessor("date", {
			header: "Accounting Period",
			cell: AppEditableCell<IBulkFleetEmissionsData>,
			meta: {
				data: {
					type: "datepicker",
					isRequired: true,
		...
		bulkColumnHelper.accessor("fleetType", {
			header: "Fleet Type",
			cell: AppEditableCell<IBulkFleetEmissionsData>,
			meta: {
				data: {
					type: "select",
					isRequired: true,
					validate: (val) => editableValidator.validateString(val, "Invalid fleet type"),
					options: [], // will be loaded dynamically onAddRow
						...
						try {
							...
							if (resp?.status === "success") {
								...
								if (Array.isArray(info)) {
									...
									tableMeta?.updateCustomOptions(row.id, "fleetCategory", options);
								}
							}
						} catch (err) {}
					},
			...
			meta: {
				data: {
					...
					async onActionSelect(table, row, ...args) {
						...
						if (!currentFleetCategory || !currentFleetType) return;
		...
		bulkColumnHelper.display({
			id: "actions",
			header: "Actions",
			cell: AppEditableTableActionBtns<IBulkFleetEmissionsData>,
		}),
	];
	...
	const onClickCalculateTotalEmissions = async () => {
		...
		const validData = data
			...
			.map((row) => {
				...
				return { date, fleetType, fleetCategory, typeOfFuel, unitOfDistance, amountOfDistance };
			});
	...
	const renderPreviewCell = useCallback((item: IBulkFleetEmissionsDataWithEmissions, columnKey: Key) => {
		const value = item[columnKey as keyof IBulkFleetEmissionsDataWithEmissions] as string | number;
	...
	return (
		<>
			<Breadcrumbs>
				...
				<BreadcrumbItem>Fleet emissions</BreadcrumbItem>
			</Breadcrumbs>
				...
				<Tabs selectedKey={selectedTab} disabledKeys={["preview"]} color="primary" onSelectionChange={(key) => onTabChange(new Set([key]))}>
					<Tab key={"add-data"} title={"Add Data"}>
						<div className="flex items-center justify-between">
							<h1 className="text-3xl font-semibold">Fleet Emissions</h1>
						...
						<div className="">
							<AppEditableTable<IBulkFleetEmissionsData>
								columns={bulkColmns}
								data={data}
								defaultData={[]}
								setData={setData}
					...
					<Tab key={"preview"} title={"Preview"}>
						...
						<AppTable<IBulkFleetEmissionsDataWithEmissions>
...
export default BulkAddFleetEmissionsData;
