...
import { BreadcrumbItem, Breadcrumbs, Button, Card, CardBody, CardFooter, CardHeader, Chip, Divider, Image, Progress, Dropdown, DropdownItem, DropdownMenu, DropdownTrigger, Avatar, Skeleton } from "@nextui-org/react";
					...
					<div className="px-3 py-3 border rounded-xl mb-2">
						...
						<div className="">
							<div className="w-full flex items-center justify-between">
								...
								<div className="flex items-center gap-2">
									{quotes?.length > 0 && <Button className="bg-green-700 text-white">Accept Quotation</Button>}
									<div className="py-3 px-4 border border-dashed border-gray-400 rounded-xl">
										<p className="text-sm text-center">{orderDetails?.orderStage === OrderStage.RFQ && "Order Pending Review By Vendor"}</p>
									</div>
								</div>
							</div>
						</div>
								...
								<Card shadow="none" className="bg-transparent border">
									...
									<CardBody>
										{rfqOrderTimelines?.length > 0 && (
											<TimelineItem title={rfqOrderTimelines?.[0]?.title} description={rfqOrderTimelines?.[0]?.description} timelineDate={format(new Date(rfqOrderTimelines?.[0]?.createdAt), "MMM dd, yyyy hh:mm bbb")} completed>
